CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_game_add`(
	IN title VARCHAR(80),
    IN description TEXT,
    IN release_date DATE,
    IN age_restriction_type INT UNSIGNED,
    IN game_console_type INT UNSIGNED,
...
BEGIN
	...
	VALUES(title, description, release_date, 3, age_restriction_type);
    ...
    INSERT INTO game(game_console_type_id, material_id)
	VALUES(game_console_type, last_material_id);
