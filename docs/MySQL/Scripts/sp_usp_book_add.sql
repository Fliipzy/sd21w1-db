CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_book_add`(
	IN title VARCHAR(80),
    IN description TEXT,
    IN isbn13 VARCHAR(13),
    IN pages INT UNSIGNED,
    IN edition VARCHAR(45),
    IN release_date DATE,
    IN creator_array LONGTEXT
)
BEGIN
	-- Declare variables
    ...
    INSERT INTO material(title, description, release_date, material_type_id)
	VALUES(title, description, release_date, 1);
    ...
    -- Try to insert the new game row
    INSERT INTO book(isbn13, pages, edition, material_id)
	VALUES(isbn13, pages, edition, last_material_id);
